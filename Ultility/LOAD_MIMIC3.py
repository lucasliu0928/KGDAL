...
def load_data9(data_dir,static_f_flag,feature_group, data_duration_type):
    ...
    overlap_df = pd.read_csv(data_dir + "Final_overlap_df.csv",index_col= 'SUBJECT_ID')  
    ...
    dbp_df = dbp_df.reindex(outcome_df['SUBJECT_ID'])
    sbp_df = sbp_df.reindex(outcome_df['SUBJECT_ID'])
    cre_df = cre_df.reindex(outcome_df['SUBJECT_ID'])
    Bicarbonate_df = Bicarbonate_df.reindex(outcome_df['SUBJECT_ID'])
    Hematocrit_df = Hematocrit_df.reindex(outcome_df['SUBJECT_ID'])
    Potassium_df = Potassium_df.reindex(outcome_df['SUBJECT_ID'])
    
    Billirubin_df = Billirubin_df.reindex(outcome_df['SUBJECT_ID'])
    Sodium_df = Sodium_df.reindex(outcome_df['SUBJECT_ID'])
    Temp_df = Temp_df.reindex(outcome_df['SUBJECT_ID'])
    WBC_df = WBC_df.reindex(outcome_df['SUBJECT_ID'])
    HR_df = HR_df.reindex(outcome_df['SUBJECT_ID'])
    RR_df = RR_df.reindex(outcome_df['SUBJECT_ID'])
    inICU_df = inICU_df.reindex(outcome_df['SUBJECT_ID'])
    onCRRT_df = onCRRT_df.reindex(outcome_df['SUBJECT_ID'])
    onHD_df = onHD_df.reindex(outcome_df['SUBJECT_ID'])
    onRRT_df = onRRT_df.reindex(outcome_df['SUBJECT_ID'])
    
    overlap_df = overlap_df.reindex(outcome_df['SUBJECT_ID'])
    ...
    train_idx = outcome_df.index[outcome_df['SUBJECT_ID'].isin(train_ID_df['SUBJECT_ID'])] #627
    test_idx = outcome_df.index[outcome_df['SUBJECT_ID'].isin(test_ID_df['SUBJECT_ID'])] #159
    external_Valid_idx = outcome_df.index[outcome_df['SUBJECT_ID'].isin(external_validation_ID_df['SUBJECT_ID'])] #159
    ...
    if(static_f_flag == True):
        ...
        Demo_df = Demo_df.reindex(outcome_df['SUBJECT_ID'])
