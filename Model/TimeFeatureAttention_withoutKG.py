...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  5 06:30:53 2021
...
#No HPO attention multiply in to feature attention with Scaled comb
class Attention_TandF_noKG(Model):
  def __init__(self, timesteps, n_feature,n_feature2,n_feature3,n_feature4,latent_dim):
    super(Attention_TandF_noKG, self).__init__()
  ...
  def call(self, inputs):
    ...
    v1 = tf.reshape(attention_f[:,0,:],[tf.shape(attention_f[:,0,:])[0],1 , tf.shape(attention_f[:,0,:])[1]]) #get the first row vector
    scale_matrix = Multiply()([attention_f, 1/v1])  #every raw divided the first row(feature atten)
