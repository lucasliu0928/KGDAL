...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 10 14:13:24 2021
...
def Trainer_1grp(input_model,train_dataset,test_dataset,EPOCHS,class_weight,outdir):
...
#Trainer 2grps
def Train(input_model,train_dataset,test_dataset,EPOCHS,class_weight,OverSample,outdir):
    #define loss and optimizer
    loss_object = tf.keras.losses.BinaryCrossentropy()
    optimizer = tf.keras.optimizers.Adam()
    ...
    #@tf.function
    def train_step(data1, data2, labels,sample_weight):
      with tf.GradientTape() as tape:
        # training=True is only needed if there are layers with different
        # behavior during training versus inference (e.g. Dropout).
        predictions = input_model(data1,data2, training=True)
        loss = loss_object(labels, predictions,sample_weight=sample_weight) #make sure labels  are also 2d ex: [128,1]
      gradients = tape.gradient(loss, input_model.trainable_variables)
      optimizer.apply_gradients(zip(gradients, input_model.trainable_variables))
    ...
    #@tf.function
    def test_step(data1,data2, labels):
      # training=False is only needed if there are layers with different
      # behavior during training versus inference (e.g. Dropout).
      predictions = input_model(data1,data2, training=False)
    ...
    for epoch in range(EPOCHS):
      ...
      for step, (X1,X2,y) in enumerate(train_dataset):
        #Oversampling to create balanced data set
        if OverSample : 
            X1,X2,y = Batch_Sampling_Funcs.oversampling_func(X1,X2,y)
            train_step(X1,X2,y)
        else:
            sample_weight = get_sample_weight(class_weight,y)
            train_step(X1,X2,y,sample_weight)
      ...
      #Run a testing loop at the end of each epoch.
      for test_data1,test_data2, test_labels in test_dataset:     
         test_step(test_data1, test_data2, test_labels)
...
def Train_4grps_withThresFeatures(input_model,train_dataset,test_dataset,EPOCHS,class_weight,OverSample,outdir):
    #define loss and optimizer
    loss_object = tf.keras.losses.BinaryCrossentropy()
    optimizer = tf.keras.optimizers.Adam()
    ...
    #@tf.function
    def train_step(data1, data2,data3,data4,data5, data6,data7,data8, labels,sample_weight):
      with tf.GradientTape() as tape:
        # training=True is only needed if there are layers with different
        # behavior during training versus inference (e.g. Dropout).
        predictions = input_model(data1,data2,data3,data4,data5, data6,data7,data8)
        labels = tf.reshape(labels,[tf.shape(labels).numpy()[0],1])
        loss = loss_object(labels, predictions,sample_weight=sample_weight) #make sure labels  are also 2d ex: [128,1]
      gradients = tape.gradient(loss, input_model.trainable_variables)
      optimizer.apply_gradients(zip(gradients, input_model.trainable_variables))
    ...
    #@tf.function
    def test_step(data1,data2,data3,data4,data5, data6,data7,data8, labels):
      # training=False is only needed if there are layers with different
      # behavior during training versus inference (e.g. Dropout).
      predictions = input_model(data1,data2,data3,data4, data5, data6,data7,data8)
      labels = tf.reshape(labels,[tf.shape(labels).numpy()[0],1])
    ...
    for epoch in range(EPOCHS):
      ...
      # Iterate over the batches of the dataset.
      for step, (X1,X2,X3,X4,X5,X6,X7,X8, y) in enumerate(train_dataset):
        #Oversampling to create balanced data set
        if OverSample : 
            X1,X2,X3,X4,y = Batch_Sampling_Funcs.oversampling_func(X1,X2,X3,X4,X5,X6,X7,X8,y)
            train_step(1,X2,X3,X4,X5,X6,X7,X8,y)
        else:
            sample_weight = get_sample_weight(class_weight,y)
            y = tf.reshape(y,[tf.shape(y)[0],1])
            train_step(1,X2,X3,X4,X5,X6,X7,X8,y,sample_weight)
      ...
      #Run a testing loop at the end of each epoch.
      for test_data1,test_data2,test_data3,test_data4,test_data5,test_data6,test_data7,test_data8, test_labels in test_dataset:     
         test_step(test_data1, test_data2,test_data3,test_data4, test_data5,test_data6,test_data7,test_data8,test_labels)
